...
// Create Channels
	...
	const subchannels = [];
	const category = await interaction.guild.channels.create({ name: `${teamName}`, type:ChannelType.GuildCategory, reason: reason });
	const promises = [
		category.children.create({ name: `${teamAcronym}-announcements`, type: ChannelType.GuildText, reason: reason, permissionOverwrites: staffTextPermissions, topic: 'Official Updates & News' }),
		category.children.create({ name: `${teamAcronym}-scheduling`, type: ChannelType.GuildText, reason: reason, permissionOverwrites: staffTextPermissions, topic: 'Practice & Event Coordination' }),
		category.children.create({ name: `${teamAcronym}-attendance`, type: ChannelType.GuildText, reason: reason, permissionOverwrites: teamTextPermissions, topic: 'Absence Notifications & Reasons' }),
		category.children.create({ name: `${teamAcronym}-team-chat`, type: ChannelType.GuildText, reason: reason, permissionOverwrites: teamTextPermissions, topic: 'General Team Discussions' }),
		category.children.create({ name: `${teamAcronym}-notes`, type: ChannelType.GuildText, reason: reason, permissionOverwrites: teamTextPermissions, topic: 'VOD Analysis & Targeted Improvement Tips' }),
		category.children.create({ name: `${teamAcronym} Team Voice`, type: ChannelType.GuildVoice, reason: reason, permissionOverwrites: teamVoicePermissions }),
		category.children.create({ name: `${teamAcronym} Coaching`, type: ChannelType.GuildVoice, reason: reason, permissionOverwrites: staffVoicePermissions }),
	];
...
module.exports = {
	...
	async execute(interaction) {
		...
		for (const roleName in roleNames) {
			await interaction.guild.roles.create({ name: `${teamName} ${roleNames[roleName]}`, reason: reason, color: color }).then(role => {
